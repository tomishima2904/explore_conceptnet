...
class TextGenerationModel(object):
    ...
    # テンプレート+刺激語+連想語 -> プロンプト作成 -> モデルへ入力して連想理由をリターン
    def text_generation(self,
                        sample_data: List[List],
                        template: str,
                        num_refs=3,
                        num_return_sequences=3):
        # 参照文を使うか使わないか (ICNLSP2023用)
        ...
        with torch.no_grad():
            for i, (sample, encoded_text) in enumerate(zip(sample_data, encoded_texts)):
                ...
                output_texts = list(map(lambda token: self.tokenizer.decode(token, skip_special_tokens=True), output_ids))

                if num_refs == 0:
                    all_output_results.append([*sample[:-1], output_texts])
                else:
                    all_output_results.append([*sample[:-1], all_references[i], output_texts])
                logger.info(f"{i+1}/{len(encoded_texts)}")

        logger.info(f"Explanations were succusessfully generated!")
        return all_output_results


    # text_generation に加えて逐次結果の出力を行う（安全性が高い，出力データを即確認したい時に有用）
...
if __name__ == "__main__":
    ...
    # 生成したテキストを見やすいように整形
    input_path = f"{text_generation_model.result_dir}/generated_texts.csv"
